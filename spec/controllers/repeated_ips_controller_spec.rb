...
RSpec.describe RepeatedIpsController, type: :controller do
  describe 'GET index' do
    ...
    let(:repeated_ips) do
      [
        { ip: '10.10.10.15', users: [users[0], users[2]] },
        { ip: '10.10.10.16', users: [users[1], users[3], users[4], users[7]] },
        { ip: '10.10.10.17', users: [users[0], users[1], users[6]] }
      ]
    end
    ...
    before do
      ...
      repeated_ips.each do |data|
        data[:users].each do |user|
          create :post, user: user, author_ip: data[:ip]
        end
      end
    end
    ...
    it 'returns ips, used by multiple users' do
      result = repeated_ips.map { |data| { ip: data[:ip], logins: data[:users].map(&:login) } }
